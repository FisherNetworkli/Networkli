...
export function SideNav() {
  ...
  useEffect(() => {
    const getUser = async () => {
      try {
        // Use getUser instead of auth.getUser for security
        ...
        // Get user roles and premium status
  ...
  const filteredNavItems = navigationItems.filter((item) => {
    // Filter for admin-only items
    if (item.adminOnly && !isAdmin) {
      return false;
    }
    
    // Filter for premium-only items
        ...
        {filteredNavItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {/* Add a small badge for premium items */}
              {item.name === 'Premium' && role === 'basic' && (
                <span className="ml-auto rounded-full bg-yellow-200 px-2 py-0.5 text-xs font-bold text-yellow-900">
                  Upgrade
                </span>
              )}
              
              {/* Add a "New" badge for new features */}
              {item.new && (
